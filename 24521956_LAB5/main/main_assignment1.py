...
from src.data_generator import get_assignment1_data
...
def run():
    X, y_true = get_assignment1_data()

    out_dir = os.path.join(ROOT, "outputs", "assignment_1")
    make_folder(out_dir)
    ...
    plot_points(X, y_true, "Bài 1 - Dữ liệu gốc", os.path.join(out_dir, "data.png"))
    plot_points(X, best["labels"], "Bài 1 - KMeans", os.path.join(out_dir, "kmeans.png"), best["centers"])
    plot_hist([r["inertia"] for r in all_results], "Bài 1 - Inertia qua nhiều seed", os.path.join(out_dir, "inertia.png"))

    summary = {
        "assignment": 1,
        "algorithm": "KMeans",
        "title": "Bài 1 - 3 cụm cân bằng",
        "comment": "Dữ liệu gồm 3 cụm có cùng số lượng điểm và cùng covariance nên KMeans phân cụm khá tốt.",
        "accuracy": acc,
        "label_map": label_map,
        "best_seed": best["seed"],
